...
(function (root, factory) {
  // Node gets it from require, the browser from the shared global scope.
  /** @type {Record<string, any>} */
  const reasoning = (typeof module !== 'undefined' && typeof require === 'function')
    ? require('./reasoning.js')
    : (root || {});
  const api = factory(reasoning.reasoningOffBody, reasoning.reasoningOnBody);
  if (typeof module !== 'undefined') module.exports = api;
  if (root) Object.assign(root, api);
})(typeof self !== 'undefined' ? self : this, function (reasoningOffBody, reasoningOnBody) {
  ...
  // Models that always think: sending thinking:{type:'disabled'} returns 400
  // ("not supported for this model"), so "none" means omitting the param
  // (anthropic.md §4). Every other Claude model must be told explicitly —
  // Opus 5 thinks by default when the param is absent.
  const ANTHROPIC_ALWAYS_THINKING = /claude-(fable|mythos)/;
  ...
  const anthropic = {
    buildRequest(req) {
      ...
      if (thinking !== 'none') {
        ...
        if (!ANTHROPIC_ALWAYS_THINKING.test(req.model)) {
          body.thinking = { type: 'disabled' };
        }
        if (!modern && typeof req.temperature === 'number') {
  ...
  /**
   * Does this model reason at all? Drives whether we send an effort level and
   * whether sampling params are allowed.
   *
   * Primarily data: the catalog records the effort levels each model accepts,
   * refreshed weekly. The name checks are the fallback for models no dataset
   * covers. Name-matching alone used to be the whole test, which silently
   * failed for every OpenRouter model — "openai/gpt-5.6-sol" matches no
   * OpenAI-shaped prefix, so the user's thinking choice was dropped.
   */
  function oaiIsReasoningModel(model, quirks, modelInfo) {
    if (modelInfo && Array.isArray(modelInfo.efforts) && modelInfo.efforts.length) return true;
    if (modelInfo && modelInfo.alwaysOn) return true;
  ...
  /**
   * Separate question: which token-cap parameter does this ENDPOINT want?
   * OpenAI deprecated max_tokens for its reasoning models, but that is an
   * OpenAI API fact, not a property of reasoning. OpenRouter documents
   * max_tokens for everything (openrouter.md §2), and only 57 of its ~400
   * models list max_completion_tokens at all.
   */
  function oaiUsesMaxCompletionTokens(model, quirks) {
    const m = String(model || '');
    const prefixes = quirks?.reasoningModelPrefixes;
    if (prefixes && prefixes.some(p => m.startsWith(p))) return true;
    return /^o[0-9]/.test(m);
  ...
  const oai = {
    buildRequest(req) {
      ...
      const reasoning = oaiIsReasoningModel(req.model, req.quirks, req.modelInfo);
      const body = {
        model: req.model,
      ...
      if (req.maxTokens) {
        // OpenAI's reasoning models reject max_tokens; everyone else takes it
        const key = oaiUsesMaxCompletionTokens(req.model, req.quirks)
      ...
      // Reasoning must always be stated, never left to the model's default:
      // many current models reason by default when the parameter is absent,
      // which on a long generation is indistinguishable from a hung request.
      const thinking = req.thinking || 'none';
      if (thinking === 'none') {
        // How to say "off" differs per provider AND per model — see
        // lib/providers/reasoning.js. null means "not documented, send nothing".
        const off = req.quirks?.thinkingOffBody
          || (reasoningOffBody ? reasoningOffBody(req.providerId, req.model, req.modelInfo) : null);
        if (off) Object.assign(body, off);
      } else if (req.quirks?.deepseekThinking) {
        // DeepSeek V4: thinking is a param, not a model (deepseek.md §4-5)
        body.thinking = { type: 'enabled' };
        body.reasoning_effort = OAI_DEEPSEEK_EFFORT[thinking] || 'high';
      } else if (reasoning) {
        // How to ASK for a level differs per provider exactly as "off" does.
        // A null answer from reasoningOnBody is a decision ("this host was
        // never confirmed to accept it"), not a gap — only fall back to the
        // OpenAI spelling when the table itself is unavailable.
        if (req.quirks?.thinkingOnBody) {
          Object.assign(body, req.quirks.thinkingOnBody);
        } else if (reasoningOnBody) {
          const on = reasoningOnBody(req.providerId, req.model, req.modelInfo, thinking);
          if (on) Object.assign(body, on);
        } else {
