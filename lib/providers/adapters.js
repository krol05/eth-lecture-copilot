...
(function (root, factory) {
  ...
  const oai = {
    buildRequest(req) {
      ...
      // Thinking must always be stated, never left to the model's default:
      // several current models reason by default when the parameter is absent,
      ...
      if (req.quirks?.deepseekThinking) {
        ...
        body.thinking = thinking === 'none' ? { type: 'disabled' } : { type: 'enabled' };
        if (thinking !== 'none') body.reasoning_effort = OAI_DEEPSEEK_EFFORT[thinking] || 'high';
      } else if (req.quirks?.thinkingOffBody && thinking === 'none') {
        // Escape hatch for any provider whose off-switch we have verified.
        Object.assign(body, req.quirks.thinkingOffBody);
      } else if (reasoning) {
        // o-series predates the none/minimal effort levels; "low" is the
        // smallest value it certainly accepts (openai.md §3).
        const isOSeries = /^o[0-9]/.test(String(req.model || ''));
        body.reasoning_effort = thinking === 'none'
          ? (isOSeries ? 'low' : 'none')
          : thinking;
      }
