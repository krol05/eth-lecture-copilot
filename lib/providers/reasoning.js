...
(function (root, factory) {
  ...
  const LOW = effort('low');          // floor where "none" is rejected
   ...
   */
  function reasoningOffBody(providerId, modelId) {
    ...
    switch (p) {
      ...
      case 'groq':
        if (IS_GPT_OSS.test(b)) return LOW;
        if (IS_QWEN.test(b)) return NONE;
        return null;                       // Llama etc. are not reasoning models

      case 'xai':
        return LOW;                        // cannot be disabled; default is "high"

      case 'mistral':
        return NONE;

      case 'together':
        return IS_GPT_OSS.test(b) ? LOW : { reasoning: { enabled: false } };

      case 'cerebras':
        if (IS_GPT_OSS.test(b)) return LOW;
        if (IS_GEMMA.test(b)) return NONE;
        return null;

      case 'fireworks':
        return IS_GPT_OSS.test(b) ? LOW : NONE;
      ...
      case 'perplexity':
        ...
        return /reasoning|deep-research/i.test(b) ? effort('minimal') : null;

      case 'cohere':
        return /reasoning/i.test(b) ? NONE : null;

      case 'zhipu':
        return /glm-5\.3/i.test(b) ? LOW : THINKING_OFF;

      case 'moonshot':
        return /kimi-k3/i.test(b) ? LOW : THINKING_OFF;
