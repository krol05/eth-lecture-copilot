...
(function (root) {
  ...
  /**
   * Pulls finished objects out of a top-level JSON array as it streams in.
   *
   ...
   *   const fresh = scanner.push(chunkText);   // only items completed just now
   *   scanner.items                            // everything so far
   *
   * The key is whatever array the response wraps its results in: "guide" for a
   * study guide, "flashcards" for a deck, "questions" for a quiz or exam paper.
   *
   * @param {string} arrayKey  the JSON key whose array is being scanned
   */
  function createJsonArrayScanner(arrayKey) {
    const items = [];
    let buffer = '';
    let searchFrom = 0;     // where to resume looking for the array opener
    let cursor = 0;         // first character not yet examined
    let started = false;    // has the `"<arrayKey>": [` opener been seen
    ...
    const OPENER = new RegExp('"' + escapeForRegExp(String(arrayKey)) + '"\\s*:\\s*\\[');
    const OVERLAP = 16;
    ...
    function push(text) {
        ...
        if (ch === '}') {
          ...
          if (depth === 0) {
            ...
            try {
              ...
              // An item that will not parse is dropped rather than retried:
              // the final non-streamed response is the authority on content.
              if (typeof root !== 'undefined' && root.CopilotDebug) {
                root.CopilotDebug.warn('jsonArrayScanner.itemParseError', {
                  arrayKey, error: err.message, objectText
                });
              }
            }
          }
      ...
      return items.slice(before);
    }

    return {
      push,
      items,
      // `blocks` is the name the guide streaming path has always used.
      blocks: items,
  ...
  /** A JSON key is author-controlled, but never trust it into a RegExp raw. */
  function escapeForRegExp(str) {
    return str.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  }

  /** The guide is one array scanner among several; this is its key. */
  function createGuideBlockScanner() {
    return createJsonArrayScanner('guide');
  }

  const api = {
    parseGuideResponse, findMatchingBrace, fixEscapes, salvageTruncated,
    createGuideBlockScanner, createJsonArrayScanner
  };
  if (typeof root !== 'undefined') {
    ...
    root.createJsonArrayScanner = createJsonArrayScanner;
  }
