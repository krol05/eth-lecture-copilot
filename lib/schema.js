...
function sanitizeGuide(guide) {
  ...
  guide.guide = guide.guide.map(block => {
    const clean = {
      ...
      key_concept_labels: Array.isArray(block.key_concept_labels)
        ? block.key_concept_labels.map(v => String(v || '').trim().slice(0, 24))
        : [],
    ...
    clean.key_concepts = sanitizeKeyConcepts(block.key_concepts, clean.key_concept_labels);
    clean.study_flow = sanitizeStudyFlow({ ...block, ...clean });
    return clean;
  });
  ...
  const lectureTitle = typeof guide.lecture_title === 'string' ? guide.lecture_title.trim() : '';
  guide.guide_title = (explicit || legacy || firstBlock || lectureTitle || 'Lecture').slice(0, 120);
