...
(function (root, factory) {
  ...
  function classify(detail) {
    ...
    if (code === 'permission_missing') {
      if (detail.raw?.origin === '<all_urls>') {
        return {
          kind: 'Attaching a frame needs one more step',
          hint: 'Chrome allows a picture of the page when you invoke the extension yourself — clicking its toolbar icon or using the shortcut both count, and both are limited to the tab you are on. A button inside the page does not count, which is why this one has to ask.'
        };
      }
    ...
    // A granted host can still 302 to one we don't hold, and the permission
    // check cannot see that: it runs against the URL we ask for, not the one
    // we are sent to. The result is an ordinary CORS failure with no clue
    // attached, so name the possibility here.
    if (status == null && detail.code === 'transcript_fetch_failed') {
      return {
        kind: 'Lecture data could not be loaded',
        hint: 'ETH serves captions from numbered storage nodes and redirects to them. If this started after an update, the extension is probably missing access to the node it was redirected to — open the service worker console to see which host was blocked.'
      };
    }
