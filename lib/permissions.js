...
(function (root, factory) {
  ...
  /**
   * Attaching a video frame screenshots the tab, and Chrome accepts only
   * `<all_urls>` or an active `activeTab` grant for that — host access to
   * video.ethz.ch is explicitly refused with "Either the '<all_urls>' or
   * 'activeTab' permission is required". activeTab only goes live when the
   * user invokes the extension from the toolbar, which never happens with a
   * sidebar injected into the page.
   *
   * So this one feature needs the broad grant. It is optional and requested
   * on the first Attach frame click, so people who never screenshot are never
   * asked, and nobody is asked at install time.
   */
