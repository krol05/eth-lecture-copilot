...
function buildFlashcardsPrompt(guide, { count = 'auto', style = 'mixed', includeFormulas = true } = {}) {
  ...
  return `You are an expert ETH Zürich professor creating Anki-style flashcards from a graduate-level lecture guide.
...
function buildQuizPrompt(guide, { count = 10, type = 'mixed' } = {}) {
  ...
  return `You are an ETH Zürich professor writing a closed-book practice exam for a graduate-level course.
...
function buildExamQuestionsPrompt(guide, selectedBlockTitles = [], {
  ...
  return `You are an ETH Zürich full professor writing exam questions for a graduate-level course.
...
function buildCrossLecturePredictionPrompt(lectures, {
  ...
  return `You are an ETH Zürich professor who has taught the full course and is designing the final exam.
