...
(function (root) {
  ...
  function enabled() {
    try {
      ...
      if (ls) return ls.getItem('eth-copilot-debug') === '1';
    } catch {}
    return root.__ETH_COPILOT_DEBUG__ === true;
  }

  function setEnabled(on) {
    try {
      const ls = root.localStorage;
      if (ls) ls.setItem('eth-copilot-debug', on ? '1' : '0');
    } catch {}
    root.__ETH_COPILOT_DEBUG__ = !!on;
  }
  ...
  root.CopilotDebug = { enabled, setEnabled, log, warn, error, sanitize };
